...
const OrderItemCard = ({ order }: Props) => {
  ...
  const handleStatusChange = async (newStatus: OrderStatus) => {//get the new status clicked on 
    await updateRestaurantStatus({//call the request with the new status
      ...
      <CardHeader>
        <CardTitle className="grid md:grid-cols-4 gap-4 justify-between mb-3">
          ...
          <div>
            Total Cost:
            <span className="ml-2 font-normal">
              DZD {(order.totalAmount / 100).toFixed(2)}
            </span>
          </div>
        </CardTitle>
        <Separator />
